...
\end{Puzzle}
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{T>{\centering\arraybackslash}p{0.2\textwidth}TC|CT}
			...
			\thead{\LaTeX \ LaTeX} &
			\thead{\tabimg{ms_word.png} Microsoft Word} \\ \Thline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{Tp{0.26\textwidth}TX|X|XT}
			...
			\end{tabitemize} & 
			\begin{tabitemize}
				\item {G++ \tabimg{GNU_Compiler_Collection_logo.png}}
				\item {MSVC\footnotemark[3] \tabimg{Visual_Studio_Icon_2019.png}}
			\end{tabitemize} &
			\begin{tabitemize}
				\item {CPython \tabimg{cython_icon.png}}
				\item PyPy \tabimg{pypy-logo.png}
			\end{tabitemize} \\ \hline
			輸出(Output) & .pdf \ \tabimg{PDF_file_icon.png} & {.exe \tabimg{windows-exe-icon-1.jpg}} & {(\ .exe \tabimg{windows-exe-icon-1.jpg}\ )} \\ \Thline
		\end{tabularx}

...
%		故事 時間線
	\section{什麼是\TeX ？}
		\TeX 是美國電腦科學家Donald Knuth在1978年發表的一套排版軟體，。相較於市面上大多的排版軟體(如: Microsoft Word、LibreOffice Writer、Google Docs)，\TeX 沒有漂亮的圖形化介面(GUI)，而是像寫程式一樣，透過指令調整文件的格局與字型樣式。
	\section{什麼是\LaTeX ？}
		隨著科技的發展，當年的\TeX 早已不敷使用

		
		既然說\LaTeX 比較像是一種程式語言，那就把他拿來與時下當紅的C++與Python來比較吧！
		
		\footnotetext[1]{Microsoft Visual C++}
		\begin{table}[H]
			\centering
			...
			\begin{tabularx}{\textwidth}{Tp{0.26\textwidth}TX|X|XT}
				...
				\begin{tabitemize}
					...
					\item {TeXstudio \tabimg{Texstudio_Logo.png}}\label{key}
				...
				\begin{tabitemize}
					...
					\item {MSVC\footnotemark[1] \tabimg{Visual_Studio_Icon_2019.png}}
				\end{tabitemize} &
				\begin{tabitemize}
					\item CPython
					\item PyPy
				\end{tabitemize} \\ \hline
				輸出(Output) & .pdf \tabimg{PDF_file_icon.png} & .exe & N/A \\ \hline
				&  & & \\
				\Thline
			\end{tabularx}
